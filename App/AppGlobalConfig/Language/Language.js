class Language {
    loadLanguage = () => {
      GLOBAL.appLanguages = {
        english: {
          logOut: 'Logout',
          login: 'Login',
          loginTab: 'Login',
          newAccount: 'Register',
          create: 'Create Account',
          accountCreated: 'New account created.',
          email: 'Email',
          name: 'Name',
          password: 'Password',
          repeat: 'Repeat Password',
          newPassword:"New Password",
          PhoneNumber:"Phone Number",
          checkFields: 'Fill in all fields.',
          loginSuccess: 'Login successful.',
          dontRemember: 'Forgot your password?',
          google: 'Login With Google',
          facebook: 'Login With Facebook',
          guest: 'Use Without Login',
          settings: 'Settings',
          mainColor: 'Main Color',
          change: 'Change',
          mainTheme: 'Main Theme',
          white: 'White',
          black: 'Dark',
          text: 'Text',
          lan: 'Language',
          language: 'English',
          hello: 'Hello',
          forget:"Request Password Reset"
        },
        arabic: {
          logOut: 'تسجيل الخروج',
          login: 'تسجيل الدخول',
          loginTab: 'دخول',
          newAccount: 'حساب جديد',
          create: 'تسجيل',
          accountCreated: 'تم تسجيل الحساب بنجاح',
          email: 'البريد الالكتروني',
          name: 'الإسم',
          password: 'كلمة المرور',
          repeat: 'اعادة كلمة المرور',
          checkFields: 'تأكد من جميع الحقول',
          loginSuccess: 'تم تسجيل الدخول بنجاح',
          dontRemember: 'هل نسيت كلمة المرور؟',
          google: 'تسجيل باستخدام جوجل',
          facebook: 'تسجيل باستخدام فيس بوك',
          guest: 'دخول كضيف',
          settings: 'إعدادات',
          mainColor: 'اللون الرئيسي',
          change: 'تغيير',
          mainTheme: 'الشكل الرئيسي',
          white: 'ابيض',
          black: 'اسود',
          text: 'كلمة',
          lan: 'اللغة',
          language: 'العربية',
          hello: 'مرحبا',
        },
        polski: {
          logOut: 'Wyloguj się',
          login: 'Zaloguj się',
          loginTab: 'Logowanie',
          newAccount: 'Nowe konto',
          create: 'Utwórz konto',
          accountCreated: 'Stworzono nowe konto.',
          email: 'Email',
          name: 'Imię',
          password: 'Hasło',
          repeat: 'Powtórz hasło',
          checkFields: 'Sprawdź wszystkie pola.',
          loginSuccess: 'Zalogowano poprawnie.',
          dontRemember: 'Nie pamiętasz hasła?',
          google: 'Zaloguj się z Google',
          facebook: 'Zaloguj się przez Facebook',
          guest: 'Używaj bez logowania',
          settings: 'Ustawienia',
          mainColor: 'Główny kolor',
          change: 'Zmień',
          mainTheme: 'Główny motyw',
          white: 'Biały',
          black: 'Ciemny',
          text: 'Tekst',
          lan: 'Język',
          language: 'Polski',
          hello: 'Witaj',
        },
      };
    }
}

export default new Language();
