...
import { View, Image, ListView, BackHandler, AsyncStorage,ImageBackground,TouchableOpacity } from "react-native";
import {
  Container,
  Left,
  Body,
...
export default class UserFriends extends Component {
  ...
  async componentWillMount() {
    ...
    BackHandler.addEventListener("hardwareBackPress", () => {
      this.props.navigation.dispatch(
        NavigationActions.reset({
          index: 0,
          actions: [NavigationActions.navigate({ routeName: "MainAppScreen" })]
        })
      );
      return true;
    });
  ...
  getMessagesMain() {
    storage.getItem(storage.keys.accessToken).then(result => {
        ...
        api.get_conversations(result).then(response => {
    ...
    globals.mainSocket.on("chat_message", data => {
      ...
      if (data.user_id == globals.user.id) {
        ...
        if (_friend != null) friends.push(_friend);
        this.setState({ friends: friends });
  ...
  wordWrittenGenerator() {
    if (this.state.last_message == undefined) {
      ...
      this.setState({
        messageWritten:
          this.state.last_message.substring(0, 30) +
          (this.state.last_message.length > 30 ? "..." : "")
      });
      console.log(this.state.messageWritten);
    }
  }
    ...
    if (this.state.screen == 1) {
      return (
        ...
        <Header
            style={{
              marginTop: "5.8%",
              backgroundColor: "rgba(11, 27, 53,0.9)",
              elevation: 0,
              marginBottom: 0
            }}
            androidStatusBarColor="rgba(11, 27, 53,0.9)"
          >
            <Body
              style={{
                flex: 2,
            ...
            <List
              ...
              renderRow={(friend, s1, index) => (
                <ListItem
                  ...
                  style={{
                    backgroundColor:
                      friend.is_seen == 1 ? "transparent" : "#eaf2ff"
                  }}
                >
                  ...
                  <Body style={{ marginLeft: "0%", width: 180, flex: 3 }}>
                    <Text
                      style={{
                        fontWeight: friend.friend.is_seen ? "normal" : "bold"
                      }}
                    >
                      {friend.friend.name}
                    </Text>
          ...
          {this.state.dataLoaded != "done" ? (
            ...
            >
              ...
              >
                {this.state.dataLoaded} messages...
              </Text>
            </View>
          ) : null}
  ...
  goToChat() {
    this.props.navigation.navigate("UserChat", {
      friend_id: this.state.currentFriend.id,
      friend_name: this.state.currentFriend.name,
      title: this.state.currentFriend.name,
      myId: this.state.user_id
    });
    setTimeout(() => {
      this.getMessagesMain(), 2000;
    });
  }
